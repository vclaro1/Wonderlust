...
(function() {
  $(function() {
    return $('#trips').imagesLoaded(function() {
      return $('#trips').masonry({
        itemSelector: '.box',
        isFitWidth: true
      });
    });
...
(function() {
  jQuery(function() {
    return $('#search').autocomplete({
      ...
      return $('<li>').data('ui-autocomplete-item', item).append('<a>' + '<span class="img">' + '<img src="' + item.image_url + '" />' + '</span>' + '<span class="title">' + item.name + '</span>' + '<span class="author">' + item.trips + " Trips" + '</span>' + '<span class="price">' + 'User since: ' + item.user_since + '</span>' + '</a>').appendTo(ul);
    };
  });
